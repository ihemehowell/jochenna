...
export default function CheckoutPage() {
  ...
  if (orderPlaced) {
    return (
      <main className="px-4 md:px-8 py-10 bg-paper min-h-screen flex items-center">
        <motion.div
        ...
        >
          <motion.div className="mb-8" variants={itemVariants}>
            ...
            <h1 className="text-4xl md:text-5xl font-semibold text-ink mb-4">Order Placed!</h1>
          ...
          <motion.div variants={itemVariants}>
            <Link href="/shop">
              <motion.button
                className="w-full bg-ink text-white py-3 rounded hover:bg-ink transition"
  ...
  if (cart.length === 0) {
    return (
      <main className="px-4 md:px-8 py-10 bg-paper min-h-screen">
        <div className="max-w-7xl mx-auto">
          <Link href="/shop" className="text-ink-soft hover:text-ink flex items-center gap-2 mb-8">
            <ArrowLeft size={18} />
            Back to Shop
          </Link>
          <div className="h-96 flex flex-col items-center justify-center">
            ...
            <Link href="/shop">
              <button className="bg-ink text-white px-6 py-3 rounded hover:bg-ink transition">
                Shop Now
              </button>
            </Link>
          </div>
    ...
    <main className="px-4 md:px-8 py-10 bg-paper">
      {showAuthPrompt && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4">
          ...
          >
            <p className="text-xs uppercase tracking-[0.22em] text-ink-soft">Checkout Account</p>
            ...
            <div className="mt-6 flex flex-col gap-3">
              <Link
                href="/auth"
                className="w-full rounded-full bg-ink px-4 py-3 text-center text-sm font-semibold text-white transition hover:bg-ink"
              >
                  Go to Login
                </Link>
                ...
                <Link
                  href="/shop"
                  className="w-full rounded-full border border-hairline px-4 py-3 text-center text-sm font-medium text-ink transition hover:bg-paper"
              >
                  Back to Shop
                </Link>
            </div>
      ...
      >
        <motion.div variants={itemVariants} className="mb-8">
          <Link href="/shop" className="text-ink-soft hover:text-ink flex items-center gap-2">
            <ArrowLeft size={18} />
            Back to Cart
          </Link>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          ...
          <motion.div className="lg:col-span-2" variants={itemVariants}>
            <div className="bg-white rounded-xl p-4 sm:p-6 md:p-8 shadow-sm">
              <h2 className="text-xl sm:text-2xl font-semibold mb-4 text-ink">Checkout</h2>

              <div className="flex flex-col gap-2 text-xs sm:text-sm text-ink-soft mb-6">
                ...
                >
                  <div>
                    ...
                    <input
                      ...
                      className="w-full border text-ink border-hairline px-4 py-3 rounded-lg outline-none focus:ring-2 focus:ring-ink focus:border-transparent transition"
                    />
                  </div>
                      ...
                      {[
                        ...
                        <motion.button
                          ...
                          className={`w-full border rounded-lg p-4 text-left transition ${
                            deliveryMethod === value ? "border-ink bg-paper ring-2 ring-ink" : "border-hairline hover:border-hairline"
                          }`}
                        >
                          <p className="font-medium text-ink">{label}</p>
                          <p className="text-xs sm:text-sm text-ink-soft mt-1">{sub}</p>
                        </motion.button>
                      ))}
                    </div>
                  </div>
                  ...
                  <motion.button
                    ...
                    disabled={loading}
                    className="w-full bg-ink text-white py-3 sm:py-4 rounded-lg hover:bg-ink disabled:bg-ink-soft transition font-medium"
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                  >
                    Continue to Payment
              ...
              {step === 2 && (
                <motion.div
                  ...
                  className="space-y-4 sm:space-y-6"
                >
                  <div className="border border-hairline rounded-lg p-4 sm:p-5 bg-paper">
                    <p className="font-semibold text-ink mb-2">Secure Paystack checkout</p>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    <motion.button
                      onClick={() => setStep(1)}
                      className="flex-1 border border-hairline py-3 text-ink rounded-lg hover:bg-paper transition font-medium"
                      whileHover={{ scale: 1.02 }}
                      whileTap={{ scale: 0.98 }}
                    >
                      Back
                    ...
                    <motion.button
                      ...
                      disabled={loading}
                      className="flex-1 bg-ink text-white py-3 rounded-lg hover:bg-ink disabled:bg-ink-soft transition font-medium"
                      whileHover={{ scale: 1.02 }}
                      whileTap={{ scale: 0.98 }}
                    >
                  ...
                  {pendingCheckoutUrl && (
                    <a
                      ...
                      className="block w-full text-center border border-hairline py-3 rounded-lg hover:bg-paper transition font-medium text-ink"
          ...
          <motion.div variants={itemVariants} className="lg:col-span-1">
            <div className="bg-white rounded-xl p-4 sm:p-6 shadow-sm lg:sticky lg:top-20">
              <h3 className="text-lg sm:text-xl font-semibold mb-4 sm:mb-6 text-ink">Order Summary</h3>

              <div className="space-y-3 max-h-64 sm:max-h-80 overflow-y-auto mb-4 sm:mb-6 pb-4 sm:pb-6 border-b">
                {cart.map((item) => (
                  <div key={`${item.id}-${item.selectedSize}`} className="flex gap-3">
                    <div className="relative w-14 h-16 shrink-0 bg-paper rounded sm:w-16 sm:h-20">
                      <Image
